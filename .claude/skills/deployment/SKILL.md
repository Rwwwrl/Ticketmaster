...
Delivery to the cluster is GitOps: **Argo CD is the only writer to the cluster**, and git is the only source of truth for what it writes. CI never touches Kubernetes. `hello_world`, `ticketmaster`, and the Cognito PreSignUp Lambda are deployed today. `frontend` still has code, a Dockerfile and a CloudFormation template, but **no active deployment** — that template is the record of what a future Kubernetes port must reproduce.
...
- `deploy/chart/`: the single Helm chart — `templates/infra/` (`external-secrets/` with the `eso` ServiceAccount and the two shared ClusterSecretStores, `ingress/` with the IngressClass, `services/hello-world/` and `services/ticketmaster/` with each service's ExternalSecret plus, for ticketmaster, its workload ServiceAccount) and `templates/services/{hello-world,ticketmaster}/` (`http/` with the Deployment + Service + Ingress, plus `ticketmaster/http/hpa.yaml`; `migrations/{expand,contract}/` with the migration Jobs), `values.yaml` (defaults) and `values.test.yaml` (test-eu)
...
- `frontend/{Dockerfile,service.yaml}`: dormant, pending a Kubernetes port
...
**Sync waves default to `0` in `values.yaml` and are overridden per environment in `values.test.yaml`.** `infra.meta.argocd.syncWave` is `-10` for test-eu; each service's `services.<service>.meta.argocd.syncWave` stays `0`; each service's `services.<service>.migrations.expand.meta.argocd.syncWave` is `-5`; each service's `services.<service>.migrations.contract.meta.argocd.syncWave` is `5` — `hello-world` and `ticketmaster` share the same four numbers today, since both follow the identical expand/rollout/contract shape. Every template reads `argocd.argoproj.io/sync-wave: {{ .Values.<tier>.meta.argocd.syncWave | quote }}` rather than hardcoding the string. The wave is a property of the template directory — everything under `templates/infra/**` (including each service's ExternalSecret, which lives at `infra/services/<service>/` rather than beside its workload) reads the infra wave; everything under `templates/services/<service>/http/**` (Deployment, Service, Ingress, and for `ticketmaster` also its ServiceAccount and HPA) reads that service's wave; each service's `migrations/{expand,contract}/` directory reads its own migration-phase wave. Do not invent per-resource waves, and change the number in `values.yaml`/`values.test.yaml`, not in individual templates. `-5` is shared across every service's expand Job and with the RabbitMQ topology Job when it lands — same wave means applied together, and each gates its own completion independently. **Waves are Application-wide, not per-service**: because `hello-world` and `ticketmaster` render into one Argo Application, a stuck or failing Job in either service's chain blocks every wave after it for both services in that sync — accepted for now since there are only two services and `hello_world` is a temporary canary. True per-service isolation would need one Argo CD Application per service (each pointing its own `path:` at the same rendered branch, e.g. via an ApplicationSet directory generator over `services/*`); that is not done here. Waves make the sync multi-step, so a Degraded resource now fails the operation and auto-sync will not retry the same commit SHA; the Application's `retry` block covers transient cases, beyond that it needs a manual Sync.
...
In CloudFormation `ValueFrom` (frontend's dormant template only), use the Secrets Manager ARN without the generated `-XxXxXx` suffix and match it in IAM with an appropriate wildcard. An ExternalSecret's `remoteRef.key` uses the plain secret/parameter name instead — no ARN involved.

`hello_world` consumes three values and `ticketmaster` consumes fourteen; the ExternalSecret/ClusterSecretStore plumbing is identical for both and is the reference for every service that follows:
...
- Each service's own `ExternalSecret` reads all of its keys — `spec.data[].sourceRef.storeRef` names a store per key (`aws-secret-store-secrets-manager`, `aws-secret-store-parameter-store`) — and writes one Kubernetes `Secret` named after the service, which its Deployment (and its two migration Jobs) consume with `envFrom`.
...
cfn-lint frontend/service.yaml
...
Read the Ingress's `status.loadBalancer.ingress[0].hostname` before probing `/readiness-check`. Chart endpoints are hyphenated (`/health-check`, `/readiness-check`) for both `hello_world` and `ticketmaster`; `frontend`'s dormant nginx config still proxies the underscored `/readiness_check`.
