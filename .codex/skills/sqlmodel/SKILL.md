...
- `text()` for raw SQL — but call it through `session.exec(text(...))`, not `session.execute(...)`. `.execute()` emits an unconditional sqlmodel deprecation warning even for `text()`; `.exec()` doesn't, runs the same statement, and just returns the raw `Result` (no `.scalars()` unwrap, since `text()` isn't a `SelectOfScalar`). The type signature of `.exec()` is narrower than its runtime, so a type-checker may flag the `text()` argument — accept that and move on.
...
## DTO Naming — `Base<Entity>DTO` for table mappings

The DTO that is a **1:1 mirror of a database table row** is named `Base<Entity>DTO` (e.g. `BaseUserDTO`, `BaseEventDTO`). It lives in `ticketmaster/schemas/dtos.py` and is what repositories return.

The `Base` prefix is load-bearing signal: at a glance, readers know this DTO represents the *table* — it's globally shareable across layers, holds every column, and is the type that crosses the repository boundary. DTOs without the `Base` prefix (e.g. `EventSummaryDTO`, `UserWithLastLoginDTO`) are local-scoped projections, partial views, or computed shapes used inside one layer.

```python
# ticketmaster/schemas/dtos.py
class BaseUserDTO(DTO):
    """Mirrors the user table — every column, returned by UserRepository."""
    id: int
    uuid: UUID
    pool_id: str
    email: str
...
If you ever introduce a narrower DTO for a specific use case, drop the `Base` prefix:

```python
class UserSummaryDTO(DTO):
    """Listing-only view of a user — no email/external_id."""
    uuid: UUID
    created_at: datetime
```
...
class EventRepository:
    @classmethod
    async def get_all(cls, session: AsyncSession) -> list[BaseEventDTO]:
        result = await session.exec(select(Event))
        return [BaseEventDTO.from_sqlmodel(event) for event in result.all()]
```
...
- **Using `.execute()` instead of `.exec()` for `text()` clauses.** `.execute()` triggers sqlmodel's "use exec()" deprecation warning regardless of the statement type. Use `.exec(text(...))` — runs identically, no warning. The type-checker complaint is the only cost.
