...
from ticketmaster.admin.redis_cache.utils import invalidate_event_cache_document_by_id
...
async def test_invalidate_event_cache_document_by_id_deletes_adjacent_version_keys(
    redis: Redis,
    ...
    for version in adjacent_versions:
        key = EventCacheRepository._cache_key(event_id=event.id, version=version)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_invalidate_event_cache_document_by_id_keeps_other_event_keys(
    redis: Redis,
    sqlmodel_engine: AsyncEngine,
) -> None:
    ...
    untouched_key = EventCacheRepository._cache_key(event_id=untouched.id, version=EventCacheDocument.version)
