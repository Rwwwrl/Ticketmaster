"""Endpoint contract tests for POST /api/v1/users/. Auth dependency is bypassed via dependency_overrides;
...
async def test_create_user_fallback_when_valid_payload_returns_201(
    ...
    response = await async_client.post(url="/api/v1/users/", json=payload)
...
async def test_create_user_fallback_when_invalid_email_returns_422(
    ...
    response = await async_client.post(url="/api/v1/users/", json={**_payload(), "email": "not-an-email"})
...
async def test_create_user_fallback_when_duplicate_email_returns_400(
    ...
    payload = _payload()
    first = await async_client.post(url="/api/v1/users/", json=payload)
    assert first.status_code == 201

    second_payload = {**payload, "uuid": str(uuid4()), "cognito_username": "bob@example.com"}
    second = await async_client.post(url="/api/v1/users/", json=second_payload)
