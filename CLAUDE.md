...
- **`ticketmaster` and `frontend`** deploy to EKS: GitHub Actions → CodeArtifact/ECR → `helm template` → orphan branch `env/test-eu` → Argo CD Application `application-test-eu` → cluster `ticketmaster-test-eu` (Auto Mode). One chart, `deploy/chart/`, with `templates/infra/` (`external-secrets/`: `eso` ServiceAccount + shared `aws-secret-store-secrets-manager`/`aws-secret-store-parameter-store` ClusterSecretStores; `ingress/`: IngressClass `alb`; `services/<service>/`: each backend's ExternalSecret; `frontend/`: the frontend's ExternalSecret), `templates/services/<service>/` (`http/`: Deployment + ClusterIP Service; `migrations/{expand,contract}/`: the expand/contract Alembic migration Jobs), `templates/frontend/` (Deployment + ClusterIP Service — nginx serving the SPA and reverse-proxying `/api/*` to the `ticketmaster` Service), a chart-root `templates/ingress.yaml` — the single shared, service-agnostic internet-facing ALB Ingress, dispatching by path prefix: `/api/*` → `ticketmaster`, everything else → `frontend`, serving `https://test-eu.as-ticketmaster.com` (ACM cert via `ingress.meta.certificateArn`, HTTP redirected to HTTPS) — and `templates/argocd/application.yaml` — the Argo CD Application itself, rendered to the env branch so it manages its own spec (sync wave `10`, deliberately last; spec changes go through the chart + a `test/**` push, never the Argo UI — see the `deployment` skill). The ALB forwards paths verbatim (no rewrite), so the backend mounts everything under `/api` (`/api/v1`, `/api/admin`, docs at `/api/docs`; only `/health-check` and `/readiness-check` stay at root — probes and target-group health checks hit pods directly). Both workload Services are ClusterIP; the frontend's nginx also keeps an unstripped `/api/*` proxy to `http://ticketmaster` as the in-cluster route. Destination namespace `default`. `ticketmaster` has its own Postgres database and an expand/contract Alembic tree; its two migration Jobs sit at sync waves `-5` (expand) and `5` (contract) either side of its Deployment's wave `0` — waves are Application-wide, so a failure in that chain stops every later wave for `frontend` too (accepted for now; see the `deployment` skill for the full mechanism and the isolation tradeoff). `ticketmaster` additionally runs its `http/` container under a `ticketmaster` ServiceAccount bound via **EKS Pod Identity** (not IRSA — that's `eso`'s mechanism) to the IAM role `ticketmaster-test-eu-ticketmaster`, which carries the AWS managed policies `AWSKeyManagementServicePowerUser` and `AmazonCognitoPowerUser`. The pod needs `kms:GetPublicKey` on the two JWT signing keys (KMS publishes no JWKS endpoint, so verifying the Lambda-issued and admin JWTs requires the API call) and `cognito-idp:AdminDeleteUser`. It also scales via a `http/hpa.yaml` HorizontalPodAutoscaler (CPU-utilization target) rather than a fixed replica count.
- **`cognito_pre_signup`** and **`s3_events`** each deploy as a Lambda via CloudFormation, using a shared parameterized workflow. Neither is on Kubernetes.
...
- **Secrets Manager ARN suffix:** an ExternalSecret's `remoteRef.key` uses the plain secret name, no ARN or suffix involved — every backend and the frontend consume secrets this way. (No CloudFormation template in the repo uses Secrets Manager `ValueFrom` any more; the Lambda template resolves its one KMS ARN via SSM's `{{resolve:ssm:...}}`, which has no ARN-suffix quirk.)
...
- **Each deployable service** (`src/ticketmaster`) declares a published version — `ticketmaster-libs = "^0.7.0"` — because its Docker build context excludes `src/libs`. The image resolves the package from CodeArtifact; the root path dep wins locally.
...
**Task runner:** Uses `just` (see `justfile` for available commands) — `just test`, `just runserver`, `just runui`, `just up-infra`. `just test-stand-down` / `just test-stand-up` fully delete and recreate the `test-eu` EKS stand (see the `deployment` skill) — user-invoked only, never run these on your own initiative.
