...
- **Secrets Manager ARN suffix:** in CloudFormation `ValueFrom` (frontend's dormant template only), drop the `-XxXxXx` suffix — IAM matches the partial ARN via wildcard at runtime. An ExternalSecret's `remoteRef.key` uses the plain secret name, no ARN or suffix involved.
