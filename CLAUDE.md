...
- **`hello_world`** deploys to EKS: GitHub Actions → CodeArtifact/ECR → `helm template` → orphan branch `env/test-eu` → Argo CD Application `application-test-eu` → cluster `ticketmaster-test-eu` (Auto Mode). One chart, `deploy/chart/`, with `templates/infra/` (`external-secrets/`: `eso` ServiceAccount + shared `aws-secret-store-secrets-manager`/`aws-secret-store-parameter-store` ClusterSecretStores; `ingress/`: IngressClass `alb`; `services/hello-world/`: the service's ExternalSecret) and `templates/services/hello-world/` (`http/`: Deployment + ClusterIP Service + internet-facing ALB Ingress; `migrations/{expand,contract}/`: the expand/contract Alembic migration Jobs), destination namespace `default`. hello_world has its own Postgres database and an expand/contract Alembic tree mirroring ticketmaster's; the two migration Jobs sit at sync waves `-5` (expand) and `5` (contract) either side of the Deployment's wave `0`, so a failure at any wave stops every wave after it — see the `deployment` skill for the full mechanism.
