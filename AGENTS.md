...
- Follow this file for always-on project guidance.
- Direct command prompts live in `.codex/commands/`.
- Project-specific reusable workflows live in `.codex/skills/<skill-name>/SKILL.md`.
  When a task matches one of those skills, read the relevant skill before
  editing or running commands.
- Keep `.codex/skills/` as the source of truth for project workflows.
- Codex runtime approvals and persistent command prefix rules live in the local
  Codex config. Project-local defaults live in `.codex/config.toml` and
  `.codex/rules/default.rules`.
- Put multi-step implementation notes in this repo's `plans/` directory when
  the user asks for a plan artifact.
